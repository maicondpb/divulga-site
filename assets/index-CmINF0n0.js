...
async function Ga() {
  ...
  const r = ze() + "/map.php";
  try {
    const x = await fetch(r, { credentials: f(341) });
    if (!x.ok) throw new Error("Erro ao carregar map.php");
    const i = await x[f(325)]();
    return (Gi = i), i;
  } catch (x) {
    return console.error(f(324), x), null;
  }
}
async function Wn(f, r = {}, x = !1) {
  const i = Rt,
    S = await Ga();
  if (!S || !S[f]) throw new Error(i(343) + f + '" não encontrada no map');
  const M = ze() + "/" + S[f],
    H = $r(),
    X = H ? { ...r, token: H } : r;
  return await (
    await fetch(M, {
      method: i(327),
      headers: { "Content-Type": i(342) },
      credentials: i(x ? 341 : 332),
